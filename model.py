...
def mpc_model(pr, index):
    # acados ocp model
    model = AcadosModel()

    # Name
    model.name = "Robot_MPC"

    # State Variables
    x = ca.SX.sym("x", pr.nx)  # x = [px, py, theta]
    pos = x[index.x_pos]
    theta = x[index.x_theta]

    # State dots
    x_dot = ca.SX.sym("x_dot", pr.nx)  # x_dot = [px_dot, py_dot, theta_dot]

    # Control Variables
    u = ca.SX.sym("u", pr.nu)  # u = [vel, omega]
    vel = u[index.u_vel]
    omega = u[index.u_omega]

    # Params
    param = ca.SX.sym("params", pr.nparam)
    ...
    # Dynamics
    dyn_f_expl = ca.vertcat(vel * ca.cos(theta), vel * ca.sin(theta), omega)
    dyn_f_impl = x_dot - dyn_f_expl

    # Cost terms
    cost_wp_pos = obj_desired_pos(pos, p_robot_pos_start, p_robot_pos_goal)
    u_normalized = ca.vertcat(vel / pr.robot_maxVel, omega / pr.robot_maxOmega)
    ...
    # model.con_h_expr_e = dis_to_obs       # triggers error
